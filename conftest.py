...
def capture_screenshots(case_name):
    ...
    if Config.NEW_REPORT is None:
        raise NameError('没有初始化测试报告目录')
    else:
        image_dir = os.path.join(Config.NEW_REPORT, "image", file_name)
        Config.driver.save_screenshot(image_dir)
...
def browser():
    ...
    elif Config.driver_type == "chrome-headless":
        # chrome headless模式
    ...
    elif Config.driver_type == "firefox-headless":
        # firefox headless模式
        firefox_options = FF_Options()
        firefox_options.headless = True
        driver = webdriver.Firefox(firefox_options=firefox_options)

    elif Config.driver_type == "grid":
        # 通过远程节点运行
